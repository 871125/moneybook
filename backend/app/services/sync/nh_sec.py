"""
NH투자증권 나무 Open API 연동 모듈

NH투자증권 오픈API 개발자센터: https://openapi.nhqv.com
...
# 자산 유형 매핑 (NH 응답의 상품구분코드 → 내부 asset_type)
_ASSET_TYPE_MAP = {
    "01": "국내주식",
    "02": "펀드",
    "03": "채권",
...
class NHSecAssetSyncer(BaseAssetSyncer):
    """NH투자증권 자산 현황 수집기 (나무 Open API, App Key + App Secret 인증)"""
    ...
    async def _fetch_isa(
        self, client: httpx.AsyncClient, token: str, account: str
    ) -> int:
        """ISA 계좌 평가금액 조회"""
        try:
            resp = await client.get(
                f"{BASE_URL}/acc/v1/balance/isa",
                params={
                    "cano": account[:8],
                    "acnt_prdt_cd": account[8:] if len(account) > 8 else "01",
                },
            ...
            body = resp.json()
            output = body.get("output", body.get("Output1", {}))
            amt_str = output.get("isa_evlu_amt", output.get("tot_evlu_amt", "0"))
            return int(str(amt_str).replace(",", "") or "0")
        except httpx.HTTPStatusError:
            return 0

    async def _fetch_irp(
        self, client: httpx.AsyncClient, token: str, account: str
    ) -> int:
        """IRP(개인형퇴직연금) 계좌 평가금액 조회"""
        try:
            resp = await client.get(
                f"{BASE_URL}/acc/v1/balance/irp",
                params={
                    "cano": account[:8],
                    "acnt_prdt_cd": account[8:] if len(account) > 8 else "01",
                },
            ...
            body = resp.json()
            output = body.get("output", body.get("Output1", {}))
            amt_str = output.get("irp_evlu_amt", output.get("tot_evlu_amt", "0"))
            return int(str(amt_str).replace(",", "") or "0")
        except httpx.HTTPStatusError:
            return 0
    ...
    async def fetch_assets(self, snapshot_date: date) -> list[dict]:
        if not self.api_key or not self.secret:
            logger.warning("NH투자 API 키 미설정 (user=%s)", self.user)
            return []
        if not self.accounts:
            logger.warning("NH투자 계좌번호 미설정 (user=%s)", self.user)
            return []
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                for account in self.accounts:
                    ...
                    # ISA 계좌
                    isa_amt = await self._fetch_isa(client, token, account)
                    if isa_amt > 0:
                        results.append({
                            "snapshot_date": snapshot_date.isoformat(),
                            "user": self.user,
                    ...
                    # IRP 계좌
                    irp_amt = await self._fetch_irp(client, token, account)
                    if irp_amt > 0:
                        results.append({
                            "snapshot_date": snapshot_date.isoformat(),
                            "user": self.user,
                ...
                logger.info(
                    "NH투자 자산 조회 완료 (user=%s, 계좌=%d개, 항목=%d건)",
                    self.user, len(self.accounts), len(results),
                )
        except httpx.HTTPStatusError as e:
            logger.error(
                "NH투자 API HTTP 오류 (user=%s): %s %s",
                self.user, e.response.status_code, e.response.text,
            )
        except Exception as e:
            logger.error("NH투자 API 오류 (user=%s): %s", self.user, e)
